...
use adb_sockets::{SocketRegistry, create_local_socket, create_remote_socket, Socket, Transport};
...
fn test_remote_to_local_flow() {
    ...
    nix::fcntl::fcntl(pair_a, nix::fcntl::FcntlArg::F_SETFL(nix::fcntl::OFlag::from_bits_truncate(flags) | nix::fcntl::OFlag::O_NONBLOCK)).unwrap();
    ...
    for _ in 0..100 {
        {
            ...
            if !pkts.is_empty() {
                let p = &pkts[0];
                if p.msg.command == 0x45545257 && p.msg.arg1 == 100 && p.payload.get_ref().as_slice() == b"hello" {
                    success = true;
                    break;
                }
            }
