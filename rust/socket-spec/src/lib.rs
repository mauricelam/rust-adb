...
fn set_tcp_keepalive(fd: RawFd, interval_sec: i32) -> Result<(), String> {
    ...
    unsafe {
        ...
        if libc::setsockopt(fd, libc::IPPROTO_TCP, TCP_KEEPIDLE, &interval_sec as *const _ as *const libc::c_void, std::mem::size_of::<libc::c_int>() as libc::socklen_t) != 0 {
            // Some platforms might not support this, but we try anyway as in C++.
        }
...
pub fn socket_spec_connect(
    _address: &str,
    _port: Option<&mut i32>,
    _serial: Option<&mut String>,
) -> Result<(), String> {
...
mod tests {
    ...
    fn test_is_local_socket_spec() {
        ...
        assert!(!is_local_socket_spec("tcp:www.google.com"));
    }
}
