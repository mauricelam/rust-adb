...
use std::env;
use sysdeps::env::{
    get_environment_variable, get_host_name_utf8, get_login_name_utf8, parse_complete_utf8,
};
...
fn test_parse_complete_utf8() {
    ...
    let all_sequences = vec![vec![], vec![0], vec![b'a']];
...
fn test_sysdeps_env() {
    ...
    assert_eq!(
        get_environment_variable("SYSDEPS_TEST_VAR"),
        Some("1".to_string())
    );
