...
pub fn wait_service(mut fd: AdbFd, serial: String, transport_id: TransportId, spec: String) {
    ...
    if components.len() < 2 {
        let _ = send_fail(&mut fd, &format!("short wait-for-: {}", spec));
        return;
    }
    ...
    let transport_type = match components[0] {
        ...
        _ => {
            let _ = send_fail(&mut fd, &format!("bad wait-for- transport: {}", spec));
            return;
        }
    };
    ...
    for component in &components[1..] {
        match *component {
            ...
            _ => {
                let _ = send_fail(&mut fd, &format!("bad wait-for- state: {}", spec));
                return;
            }
        }
    }
...
pub fn connect_service(mut fd: AdbFd, host: String) {
    ...
    let _ = send_protocol_string(&mut fd, &response);
}
...
impl Socket for SmartSocket {
    ...
    fn enqueue(&self, data: Bytes) -> i32 {
        ...
        fdevent.run_on_looper(move |fdevent| {
            ...
            // Downcast peer to LocalSocket if possible to get its AdbFd.
...
fn open_pty() -> std::io::Result<(AdbFd, AdbFd)> {
    unsafe {
        ...
        Ok((
            AdbFd::from_raw_fd(master_fd),
            AdbFd::from_raw_fd(slave_fd),
        ))
    }
}
...
pub fn shell_service(adb_fd: AdbFd, args: &str) {
    ...
    let mut sub_stdin_fd = if is_pty {
        ...
        #[cfg(unix)]
        {
            child.stdin.take().map(|s| unsafe { AdbFd::from_raw_fd(s.into_raw_fd()) })
        }
        #[cfg(windows)]
        {
            child.stdin.take().map(|s| unsafe { AdbFd::from_raw_handle(s.into_raw_handle() as _) })
        }
    };
    let mut sub_stdout_fd = if is_pty {
        ...
        #[cfg(unix)]
        {
            child.stdout.take().map(|s| unsafe { AdbFd::from_raw_fd(s.into_raw_fd()) })
        }
        #[cfg(windows)]
        {
            child.stdout.take().map(|s| unsafe { AdbFd::from_raw_handle(s.into_raw_handle() as _) })
        }
    };
    let mut sub_stderr_fd = if is_pty {
        ...
        #[cfg(unix)]
        {
            child.stderr.take().map(|s| unsafe { AdbFd::from_raw_fd(s.into_raw_fd()) })
        }
        #[cfg(windows)]
        {
            child.stderr.take().map(|s| unsafe { AdbFd::from_raw_handle(s.into_raw_handle() as _) })
        }
    };
            ...
            if pfds.len() > 1 && pfds[1].revents & POLLIN != 0 {
                ...
                if let Some(ref mut f) = sub_stdout_fd {
                    match f.read(&mut buf) {
                        Ok(n) if n > 0 => {
                            if is_v2 {
                                ShellProtocol::write_packet(&mut adb_fd, ShellId::Stdout, &buf[..n])
                                    .unwrap();
                            } else {
            ...
            if pfds.len() > 2 && pfds[2].revents & POLLIN != 0 {
                ...
                if let Some(ref mut f) = sub_stderr_fd {
                    match f.read(&mut buf) {
                        Ok(n) if n > 0 => {
                            if is_v2 {
                                ShellProtocol::write_packet(&mut adb_fd, ShellId::Stderr, &buf[..n])
                                    .unwrap();
                            } else {
                                adb_fd.write_all(&buf[..n]).unwrap();
                            }
...
pub fn daemon_service_to_fd(name: &str, _transport: &Arc<ATransport>) -> Option<AdbFd> {
...
mod integration_tests {
    ...
    fn test_create_service_thread() {
        let fd = create_service_thread("test", |mut fd| {
    ...
    fn test_smartsocket_dispatch_host_service() {
        ...
        *t.serial.lock().unwrap() = serial.to_string();
        ...
        smart_socket.enqueue(Bytes::from("000Ahost:bogus"));
