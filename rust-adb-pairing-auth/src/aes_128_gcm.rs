...
impl Aes128GcmCipher {
    ...
    pub fn encrypt(&mut self, data: &[u8]) -> Result<Vec<u8>, Aes128GcmError> {
        ...
        let result = cipher.encrypt(&nonce, data).map_err(|_| Aes128GcmError::EncryptionFailed)?;
        self.enc_sequence += 1;
        Ok(result)
    }
    ...
    pub fn decrypt(&mut self, data: &[u8]) -> Result<Vec<u8>, Aes128GcmError> {
        ...
        let result = cipher.decrypt(&nonce, data).map_err(|_| Aes128GcmError::DecryptionFailed)?;
        self.dec_sequence += 1;
        Ok(result)
    }
}
